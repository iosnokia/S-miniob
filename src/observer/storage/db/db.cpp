...
RC Db::create_table(const char *table_name, int attribute_count, const AttrInfoSqlNode *attributes)
{
  RC rc = RC::SUCCESS;
  // check table_name
  if (current_tables_.count(table_name) != 0) {
  ...
  current_tables_[table_name] = table;
  LOG_INFO("Create table success. table name=%s, table_id:%d", table_name, table_id);
  return RC::SUCCESS;
}
RC Db::drop_table(const char *table_name)
{
  auto it = current_tables_.find(table_name);
  if(it == current_tables_.end()){
    return RC::SCHEMA_TABLE_NOT_EXIST;
  }
  Table *table = it->second;
  RC rc = table->destroy(path_.c_str());
  if(rc != RC::SUCCESS) return rc;
  current_tables_.erase(it);
  delete table;
  return RC::SUCCESS;
}
...
{
  std::unordered_map<std::string, Table *>::const_iterator iter = current_tables_.find(table_name);
  if (iter != current_tables_.end()) {
    return iter->second;
  }
  return nullptr;
}
...
RC Db::open_all_tables()
{
  ...
  for (const std::string &filename : table_meta_files) {
    ...
    if (current_tables_.count(table->name()) != 0) {
      delete table;
    ...
    current_tables_[table->name()] = table;
    LOG_INFO("Open table: %s, file: %s", table->name(), filename.c_str());
  }

  LOG_INFO("All table have been opened. num=%d", current_tables_.size());
  return rc;
}
...
{
  for (const auto &table_item : current_tables_) {
...
{
  RC rc = RC::SUCCESS;
  for (const auto &table_pair : current_tables_) {
